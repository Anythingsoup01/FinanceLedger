...
void LedgerPopup::Render() {
  switch (m_LedgerPopupType) {
    case LedgerPopupType::CreateTable: {
      RenderCreateTablePopup();
      break;
    }
    case LedgerPopupType::SaveAndExit:
    case LedgerPopupType::SaveAndOpenExistingTables: {
      RenderSavePopup();
      break;
    }
    case LedgerPopupType::EntryDetails: {
      RenderEntryDetailsPopup();
      break;
    }
    case LedgerPopupType::TableDetails: {
...
void LedgerPopup::ViewEntry(const int &tableID, const int &entryID, const bool &isEntryCredit) {
  m_ViewingTableID = tableID;
  m_ViewingEntryID = entryID;
  m_IsViewingCreditEntry = isEntryCredit;

  m_LedgerPopupType = LedgerPopupType::EntryDetails;
}

void LedgerPopup::ViewTable(const int &tableID) {
  m_ViewingTableID = tableID;
  m_LedgerPopupType = LedgerPopupType::TableDetails;
}

void LedgerPopup::RenderCreateTablePopup() {
  ...
  if (ImGui::Button("Confirm")) {
    ...
    m_LedgerPopupType = LedgerPopupType::NONE;
  }
...
void LedgerPopup::RenderSavePopup() {
  ImGuiViewport *viewport = ImGui::GetMainViewport();
  ImGuiWindowFlags flags = ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoResize;
  // Sets the window size to be a third of the application window size
  float windowSizeX = viewport->Size.x / 3.0f;
  ...
  if (ImGui::Button("Yes")) {
    ...
    if (m_LedgerPopupType == LedgerPopupType::SaveAndExit)
      Application::Get().OnApplicationExit();

    if (m_LedgerPopupType == LedgerPopupType::SaveAndOpenExistingTables)
...
void LedgerPopup::RenderEntryDetailsPopup() {
  ImGuiViewport *viewport = ImGui::GetMainViewport();
  ImGuiWindowFlags flags = ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoResize;
  // Sets the window size to be a third of the application window size
  float windowSizeX = viewport->Size.x / 3.0f;
...
void LedgerPopup::LoadEntriesToTable(AccountTable *toTable, const EntryTable &fromEntryTable) {
...
void LedgerPopup::RenderTableDetailsPopup() {
  ImGuiViewport *viewport = ImGui::GetMainViewport();
  ImGuiWindowFlags flags = ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoResize;
  // Sets the window size to be a third of the application window size
  float windowSizeX = viewport->Size.x / 3.0f;
